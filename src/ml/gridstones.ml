...
let matches_any_rot board_state pattern =
  ...
  get_all_rots pattern |. List.map matches_cur_board |. List.some (fun isMatch -> isMatch)
  
let matches_any_pattern board_state patterns = 
  mapWithIndex patterns (fun i pattern -> ( i, matches_any_rot board_state pattern)) |. keep (fun (_, hasMatch) ->  hasMatch)

let modify_slot board_state position state = 
  let set_cur_board x y value = set_unsafe board_state x y value in
  match state with 
  | EMPTY -> set_cur_board position.x position.y 1; NEW
  | PLACED -> set_cur_board position.x position.y 0; EMPTY
  | NEW -> NEW
  | LOCKED -> LOCKED
